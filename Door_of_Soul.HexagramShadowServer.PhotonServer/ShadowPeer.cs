...
namespace Door_of_Soul.HexagramShadowServer.PhotonServer
{
    public class ShadowPeer : InboundS2SPeer
    {
        ...
        public ShadowPeer(InitRequest initRequest) : base(initRequest)
        {
            ...
            if (ShadowHexagramEntranceFactory.Instance.CreateEntrance(hexagramEntranceId, SendEvent, SendOperationResponse, out entrance))
            {
                Entrance = entrance;
            }
            else
            {
                throw new Exception("ShadowPeer CreateEntrance Fail");
            }
        }
        ...
        protected override void OnOperationRequest(OperationRequest operationRequest, SendParameters sendParameters)
        {
            ...
            string errorMessage;
            if (!EntranceCommunicationService<ShadowEventCode, ShadowOperationCode, VirtualShadow>.Instance.HandleOperationRequest(Entrance, VirtualShadow.Instance, operationCode, parameters, out errorMessage))
            {
